...
// Force dynamic so it fetches every time
export const dynamic = 'force-dynamic';
export const revalidate = 0;
...
const TutorDetailPage = async ({ params }) => {
  ...
  // 2. DEBUG: If data is empty, the screen will tell us exactly why
  if (!tutor || !tutor.tutorName) {
    return (
      <div style={{ padding: '50px', background: 'white', color: 'black' }}>
        <h1>Data Fetch Failed</h1>
        <p>We reached the server, but received an empty or invalid object:</p>
        <pre>{JSON.stringify(tutor, null, 2)}</pre>
        <p><strong>URL Accessed:</strong> {`${process.env.NEXT_PUBLIC_SERVER_URL}/tutor/${id}`}</p>
      </div>
    );
  }

  // 3. If data exists, render your normal page
  return (
